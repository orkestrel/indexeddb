...
| `IndexedDBUpgradeStoreManagerInterface` | interface | The upgrade's store manager (`names` / `create` / `drop` / `open`).                                |
...
| `IndexedDBRecordStoreInterface`         | interface | The keyed record surface the store contracts share, declared once.                                 |
| `IndexedDBStoreInterface`               | interface | The object-store contract — the record surface plus schema metadata and `index`.                   |
...
| `open`   | `IndexedDBTransactionStoreInterface` | A transaction-bound store for data migration.            |
...
const db = createIndexedDBDatabase({
	...
	upgrade: async (context) => {
		...
		// Migrate data in place, awaiting only the IDB requests `open()` issues.
...
- [`tests/src/browser/IndexedDBCursor.test.ts`](../tests/src/browser/IndexedDBCursor.test.ts) — the store/index cursor: the position snapshot (`key` / `primary` / `value` / `direction`), the moves (`continue` / `seek` / `advance`), in-place `update` / `delete`, an index cursor's `update` / `delete` rejecting with `READONLY`, and a non-record stored value reading `undefined` from `value`.
